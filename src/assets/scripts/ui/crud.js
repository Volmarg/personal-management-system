...
export default (function () {
    ...
    ui.crud = {
        ...
        ajaxToggleLockRecord: function (tr_parent_element) {
            ...
            let url = ui.crud.general.methods.removeEntity.lockResource.url.replace(ui.crud.general.methods.removeEntity.lockResource.params.record, record);
                url = url.replace(ui.crud.general.methods.removeEntity.lockResource.params.type, type);
                url = url.replace(ui.crud.general.methods.removeEntity.lockResource.params.target, target);

            bootbox.confirm({
                message: "Do you want to lock this resource",
                backdrop: true,
                callback: function (result) {
                    if (result) {

                        $.ajax({
                            method: ui.crud.general.methods.removeEntity.lockResource.method,
                            url   : url
